...
def agregar_tren():
    try:
        ...
        #  VALIDAR QUE LA COCHERA NO ESTÉ OCUPADA  👈 AQUÍ VA
        cur.execute("""
            SELECT COUNT(*) 
            FROM tren 
            WHERE id_cochera = %s
        """, (id_cochera,))
        ocupada = cur.fetchone()[0]

        if ocupada > 0:
            return jsonify({'error': 'La cochera ya tiene un tren asignado'}), 400

        # VALIDAR REGLAS DE LÍNEA
        if id_linea:
        ...
        # INSERTAR (DESPUÉS DE TODAS LAS VALIDACIONES)
        cur.execute("""
